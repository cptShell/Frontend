		...
		>
			...
			<div className="relative">
				<input
					...
					type="text" className={errors?.name ? "authorization-block__password form__input relative" +
						" border-red-500 mb-8" : "authorization-block__password form__input relative mb-8"}
			...
			<div className={"relative"}>
				<input
					...
					type={checkPassword ? 'text' : 'password'} className={errors?.password ? "authorization-block__password form__input relative mb-8" +
						" border-red-500" : "authorization-block__password form__input relative mb-8"} placeholder="Password" />

				{errors.password &&
					<p className={"absolute bottom-[-22px] left-1 text-md text-red-500 font-medium"}>{errors?.password?.message || "Error!"}</p>}

				<span onClick={clickCheckPassword} className={"absolute right-4 top-2 bottom-0 cursor-pointer"}>
					{checkPassword
						?
